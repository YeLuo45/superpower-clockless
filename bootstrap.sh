...
# Generate default API key (32 hex chars)
generate_api_key() {
...
# Write env file with auto-generated API key
write_env() {
    API_KEY=$(generate_api_key)
    ENV_FILE="$HOME/.superpower-clockless/env"
    mkdir -p "$(dirname "$ENV_FILE")"
    echo "[bootstrap] Auto-generated API key for this install."
...
# ai-superpower API key — auto-generated for local development
# To use with ai-superpower server, set the same key in ai-superpower config
...
main() {
    ...
    echo ""
    echo "Your auto-generated API key:"
    grep AI_SUPERPOWER_API_KEY "$HOME/.superpower-clockless/env" | sed 's/.*="//;s/"$//'
    echo ""
    echo "1. Use this key when setting up ai-superpower server:"
    echo "   Set the same key in your ai-superpower config (same key on both sides)"
    echo ""
