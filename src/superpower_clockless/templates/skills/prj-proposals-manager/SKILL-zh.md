...
**所有项目/提案数据操作必须通过 ai-superpower API 进行，禁止直接读写 CSV。**

**API endpoint**: ai-superpower server（默认 `http://0.0.0.0:8000`）  
**API key**: 从环境变量 `SUPERPOWER_API_KEY` 读取

> 设置一次：Linux/macOS: `export SUPERPOWER_API_KEY="your-key-here"`（写入 `~/.bashrc` 永久生效）
...
api = ProjectsAPI(api_key=os.environ["SUPERPOWER_API_KEY"])
...
api.delete("PRJ-20260523-001")   # 需 allow_delete=true
```
...
api = ProposalsAPI(api_key=os.environ["SUPERPOWER_API_KEY"])

api.list(project_id="PRJ-20260523-001", status="in_dev", search="keyword")
api.get("P-20260523-001")
api.create(title="提案标题", owner="owner", project_id="PRJ-20260523-001", stage="ideation")
api.update_fields("P-20260523-001", title="新标题", stage="development")
api.update_status("P-20260523-001", status="in_dev")   # 状态机流转
api.delete("P-20260523-001")   # 需 allow_delete=true
```
...
| 变量 | 说明 |
|------|------|
| `SUPERPOWER_API_KEY` | API 密钥（从 `~/.ai-superpower/config.toml` 中复制过来） |
| `SUPERPOWER_ROOT` | 根目录，默认为 `/home/hermes/proposals` |
