...
# Generate a default API key (32 hex chars)
function New-ApiKey {
    return [guid]::NewGuid().ToString("N")
}

# Write env.bat with a pre-generated API key
...
REM ai-superpower API key — pre-generated for local development
REM To use with ai-superpower server, set the same key in ai-superpower config
...
$DEFAULT_API_KEY = New-ApiKey
Write-Host "[installer] Auto-generated API key for this install." -ForegroundColor Yellow

try {
    Clone-Repo
    $ok = Install-Local
    if (-not $ok) { throw "Install-Local failed" }
    Write-EnvBat -ApiKey $DEFAULT_API_KEY
} catch {
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "Your auto-generated API key:" -ForegroundColor White
    Write-Host "  $DEFAULT_API_KEY" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "1. Use this key when setting up ai-superpower server:" -ForegroundColor White
    Write-Host "   Set the same key in your ai-superpower config (same key on both sides)" -ForegroundColor Gray
    Write-Host ""
    Write-Host "2. Run in a new terminal:" -ForegroundColor White
    Write-Host "   .\.superpower-clockless\env.bat" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "3. Verify the install:" -ForegroundColor White
